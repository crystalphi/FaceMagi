			...
			<table cellpadding="5">
				<div class="mc-head">
					<div class="mc-head-box">
						<div class="mc-week">周</div>
						<div v-for="(week, index) in weeks" :key="index" class="mc-week">{{week}}</div>
					</div>
				</div>
				<div class="mc-body">
					<tr v-for="(day,k1) in days" :key="k1" :class="{'gregorianStyle': !lunar}">
						<td class="mc-day" :style="itemStyle">
							<block v-if="firstWeekOfMonth_.length>k1">
								<span>{{firstWeekOfMonth_[k1]}}</span>
							</block>
						</td>
...
<script>
	...
	export default {
		...
		watch: {
			firstWeekOfMonth(val) {
				// console.log('firstWeekOfMonth:'+val)
				this.firstWeekOfMonth_ = val;
		...
		mounted() {
			...
			const itemWidth = (calendar.clientWidth / 8 - 4).toFixed(5) + 'px';
			this.itemStyle = {
				width: itemWidth,
				height: itemWidth,
		...
		methods: {
			init() {
				...
				this.$emit('change', this.month + 1, this.year);

				this.render(this.year, this.month);
			},
			// 渲染日期
			render: function(y, m, renderer) {
				if (renderer) {
					this.year = y;
					this.month = m;
			...
			renderer(y, m) {
				...
				this.$emit('change', renderM + 1, renderY);
