<template>
	<view>
		...
		<view class="main-image">
			<image :src="mainImage" :data-src="mainImage" mode="aspectFill" />
		</view>

		<scroll-view scroll-x class="bg-white nav opacity-all text-shadow" scroll-with-animation :scroll-left="tabScrollLeft">
			<view class="cu-item" :class="index==tabIndex?'text-green cur':''" v-for="(tab,index) in tabSwiperData" :key="index"
			 @tap="tabSelect" :data-id="index">
				{{tab.name}}
			</view>
		</scroll-view>
		
		<view class="gallerypanel opacity-all" style="filter:brightness(1);border-radius:1upx" :style="{'background':bkstyle,'height':galleryheight+'px','width':'100%'}"
		 :current="tabIndex" :duration="200">
			 ...
			 :scroll-left="scrollLeft" @scroll="scroll">
				...
				<view class="scroll-view-panle" @touchend='touchEndHandle' @touchstart='touchStartHandle' @touchmove='touchMoveHandle'>
					...
					<view :id="'imgviewpanel'+index" :ref="'imgviewpanel'+index" :class="[activeviewindex == index? 'trans-fadeout':  '']"
					 @animationstart="animationstartHandle" @animationend="animationendHandle" v-for="(item,index) in sviewlst.imgsview"
					 :key="index" :style="{'width':item.w+'px','height':galleryheight+'px'}" style="border: #BB6622 0px solid;display:flex;flex-direction: column;align-content:center;justify-content: flex-end;">
						<!-- 内层 高度是图片容器的高度 -->
						<!-- 顶部圆角角标 -->
						<view v-if="showbadge && item.badeg!=''" class="roundbadge" :style="{background:item.badegcolor}">
							<text>{{item.badeg}}</text>
						</view>
						<view class="trianglePanel" style="max-width:50px, min-width:50px, max-height:50px, min-height:50px">
							<view class="triangle-topright" style="border-top:50px solid blue, border-left:50px solid transparent"></view>
							<!-- 用margin-left重叠三角 -->
							<view class="textbox" style="width:25px, height:25px, font-size:6px, margin-left:-25px">
							</view>
						</view>
						<!-- 图片框 居中属性-->
						<view style="display:flex; align-content:flex-end;justify-content: center; border:green 0px solid;" :style="{'width':item.w+'px','height': item.h+'px'}">
							<image :ref="'image'+index" style="align-self:center; border-radius:4px; opacity:1.0;" :style="{'width' :item.w-10 +'px',
												'height':item.h-10 +'px',
												'border':[activeviewindex == index? '#EC6D2C 1px solid':'#EC6D2C 0px solid']}"
							 :src="item.url" @click="clickimg(index,item)" mode="scaleToFill">
							</image>
						</view>
						<!-- 描述视图 -->
						<view style="display: flex;justify-content: center;margin-bottom: 4px;border:  #008000 0px solid;">
							<text v-if="showdec" style="align-self: center;font-size: 7px;color:white">{{item.dec}}</text>
						</view>
					</view>
		...
		<view class="cu-bar tabbar bg-white">
			<view class="action text-green">
				<view class="cuIcon-down"></view>{{ i18n.save }}
			</view>
			<view class="action text-green add-action">
				<button class="cu-btn cuIcon-camerafill bg-green shadow" @tap="choosePhoto"></button>Photo
			</view>
			<view class="action text-green">
				<view class="cuIcon-forward"></view>{{ i18n.forward }}
			</view>
		</view>
...
<script>
	export default {
		...
		watch: {
			async images() {
				var that = this;
				//console.log('watch监控到images属性变化。屏幕中点pos位置:' + this.screencenterpos + 'px');
				this.screencenterpos = parseInt(uni.upx2px(750) / 2);
				this.upDataScrollviewLayout(this.activeviewindex);
			},
		},
		...
		methods: {
			// 根据选择的标签页设置组件当前显示的图片列表
			setimgs(tabIndex) {
				let imgs = [];
				this.images = this.tabSwiperData[tabIndex].images;
				console.log('--> setimgs()# this.images: ' + this.images);
				for (let i in this.images) {
					...
					imgobj.url = this.images[i].url;
					imgobj.id = this.images[i].id;
					imgobj.is_pro = this.images[i].is_pro;
					imgobj.dominant = '#' + ('00000' + (Math.random() * 0x1000000 << 0).toString(16)).substr(-6); //随机主色
					imgobj.dec = i; //描述
					imgobj.badeg = imgobj.is_pro ? 'PRO' : ''; //角标文字
					imgobj.badegcolor = 'red'; //角标颜色
					imgs.push(imgobj)
				}
				this.images = imgs;
			},
			tabSelect(e) {
				...
				if (this.tabIndex === tabIndex) {
					...
					console.log('切换到分组标签编号 ' + this.tabIndex);
					this.tabScrollLeft = (tabIndex - 1) * 60;
					this.activeviewindex = 0; //每组图片滤镜默认用第一个
					this.setimgs(this.tabIndex);
				}
			},
			/*
			getElSize(id) { //得到元素的size
				return new Promise((res, rej) => {
					uni.createSelectorQuery().select("#" + id).fields({
						...
						console.log('--> getElSize(): data:' + data)
						res(data);
					}).exec();
				})
			},
			async tapTab(e) { //点击tab-bar
				...
				console.log('--> tapTab(): tabIndex:' + tabIndex);
				if (this.tabIndex === tabIndex) {
					return false;
				} else {
					let tabBar = await this.getElSize("tab-bar"),
						tabBarScrollLeft = tabBar.scrollLeft; //点击的时候记录并设置scrollLeft
					console.log('--> getElSize(): tabBar:' + tabBar);
					this.tabScrollLeft = tabBarScrollLeft;
			...
			*/
			...
			chooseImage: async function(sourceType, sizeType) {
				uni.chooseImage({
					count: 1, //默认9
					sourceType: sourceType,
					sizeType: sizeType,
					success: (res) => {
						this.mainImage = res.tempFilePaths;
					}
				})
			},
			chooseAlbum: function(sourceType, sizeType) {
				this.chooseImage(['album'], ['compressed']);
			},
			chooseCamera: function(sourceType, sizeType) {
				this.chooseImage(['camera'], ['compressed']);
			},
			choosePhoto: function(sourceType, sizeType) {
				this.chooseImage(['camera', 'album'], ['compressed']);
			},
			...
			clickimg(idx, imgviewobj) {
				...
				this.scrollLeft = this.sviewlst.imgsview[idx].l - parseInt(this.screencenterpos) + parseInt(this.sviewlst.imgsview[
					idx].w / 2);
				this.$emit('clickimg', imgviewobj, this.mainImage);
			},
...
<style lang="scss" scoped="">
	...
	.opacity-all {
		opacity: 0.88; //值越小越透明，会应用到子元素
	}
	.opacity-self {
		background: rgba(255, 255, 255, 0.8); //不值越小越透明，不会应用到子元素
	}
