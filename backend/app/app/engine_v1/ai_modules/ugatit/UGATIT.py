...
class UGATIT(object) :
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            trainA = trainA.apply(prefetch_to_device(gpu_device, None))
            ...
            trainB = trainB.apply(prefetch_to_device(gpu_device, None))
    ...
    def test_image(self, image_pil):
        from app.engine_v1.utils import image_pil_to_cv2, image_cv2_to_pil
        from app.engine_v1.utils import cv2_normalize, cv2_sharpen_gaus, cv2_sharpen_kernel
        
        def __load_image(image_pil, size=256):
            img = image_pil_to_cv2(image_pil)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            if img.shape[0] != size:
                img = cv2.resize(img, dsize=(size, size))
            #img = cv2_sharpen(img) # 锐化输入图，会使输出图更接近写实风格
            img = np.expand_dims(img, axis=0)
            img = img/127.5 - 1
            return img
        
        sample_image = np.asarray(__load_image(image_pil))
        # A -> B
        _img_fake = self.sess.run(self.test_fake_B, feed_dict={self.test_domain_A:sample_image})
        #save_images(_img_fake, [1, 1], '/app/app/debug.png')
        _img_fake = inverse_transform(_img_fake)
        #print('--> type(_img_fake):', type(_img_fake))

        _img_out = merge(_img_fake, [1, 1])
        _img_out = cv2.cvtColor(_img_out.astype('uint8'), cv2.COLOR_RGB2BGR)

        # 锐化图像
        #_img_out = cv2_sharpen_gaus(_img_out) # 不佳，会有较粗的轮廓
        #_img_out = cv2_sharpen_kernel(_img_out) # 某些情况较好
        # 增加对比
        _img_out = cv2_normalize(_img_out) # 较好
        
        return image_cv2_to_pil(_img_out)
