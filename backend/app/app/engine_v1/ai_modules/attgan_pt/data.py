...
class MyTestData(data.Dataset):
    def __init__(self, image, attr, image_size):
        self.image = image
        
        self.labels = np.array([attr], dtype=np.int)
        print('--> self.labels: %s, len: %d' % (self.labels, len(self.labels)))
    ...
    def __getitem__(self, index):
        img = self.tf(Image.fromarray(self.image))
...
class Custom(data.Dataset):
    def __init__(self, data_path, attr_path, image_size, selected_attrs):
        ...
        print('--> self.images: %s, len: %d' % (self.images, len(self.images)))
