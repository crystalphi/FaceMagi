...
# 对于这样需要直接执行的程序，需要在搜索路径中指定加入当前目录，否则会有找不到模块的报错
#current_dir = os.path.abspath(os.path.dirname(__file__))
#sys.path.append(current_dir)
sys.path.append("..")

from attgan_pt.attgan import AttGAN
from attgan_pt.data import check_attribute_conflict
from attgan_pt.helpers import Progressbar
from attgan_pt.util import find_model
...
def parse(args=None):
    ...
    parser.add_argument('--test_atts', dest='test_atts',
                        nargs='+', help='test_atts')
    parser.add_argument('--test_ints', dest='test_ints',
                        type=float, nargs='+', help='test_ints')
    ...
    parser.add_argument('--custom_attr', type=str,
                        default='./data/list_attr_custom.txt')
...
if __name__ == '__main__':
    ...
    assert args.test_atts is not None, 'test_atts should be chosen in %s' % (
        str(args.attrs))
    for a in args.test_atts:
        assert a in args.attrs, 'test_atts should be chosen in %s' % (
            str(args.attrs))

    assert len(args.test_ints) == len(
        args.test_atts), 'the lengths of test_ints and test_atts should be the same!'

    if args.custom_img:
        output_path = join('output', args.experiment_name,
                           'custom_testing_multi_' + str(args.test_atts))
        from data import Custom
        test_dataset = Custom(
            args.custom_data, args.custom_attr, args.img_size, args.attrs)
    else:
        output_path = join('output', args.experiment_name,
                           'sample_testing_multi_' + str(args.test_atts))
        if args.data == 'CelebA':
        ...
        if args.data == 'CelebA-HQ':
            ...
            test_dataset = CelebA_HQ(args.data_path, args.attr_path,
                                     args.image_list_path, args.img_size, 'test', args.attrs)
    ...
    attgan.load(find_model(
        join('output', args.experiment_name, 'checkpoint'), args.load_epoch))
