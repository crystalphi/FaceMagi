...
class adaLIN(nn.Module):
    ...
    def forward(self, input, gamma, beta):
        ...
        #in_mean, in_var = input, input
        #for dim in [2, 3]:
        #    in_mean, in_var = torch.mean(in_mean.clone().detach(), dim, keepdim=True), torch.var(in_var.clone().detach(), dim, keepdim=True)
        out_in = (input - in_mean) / torch.sqrt(in_var + self.eps)
        ln_mean, ln_var = torch.mean(input, dim=[1, 2, 3], keepdim=True), torch.var(input, dim=[1, 2, 3], keepdim=True)
        #ln_mean, ln_var = input, input
        #for dim in [1, 2, 3]:
        #    ln_mean, ln_var = torch.mean(ln_mean.clone().detach(), dim, keepdim=True), torch.var(ln_var.clone().detach(), dim, keepdim=True)
