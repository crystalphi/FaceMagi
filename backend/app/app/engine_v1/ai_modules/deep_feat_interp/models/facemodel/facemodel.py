...
class FaceModel:
    def __init__(self, modelfile='model.t7'):
        ...
        self.model.load_state_dict(torch.load('models/facemodel/model_cpu.pth', map_location="cuda:0"))
        self.model.eval()  # set model to test mode
        self.model.cuda()
        self.meanstd = {'mean': numpy.array(
            [0.485, 0.456, 0.406]), 'std': numpy.array([0.229, 0.224, 0.225])}
        self.celeba_template = numpy.load(
            'models/facemodel/celeba_dlib_template.npz')['template']
        self.clf = {k: pickle.load(open(
            'models/facemodel/classifiers/{}.pkl'.format(k), 'rb'), encoding='iso-8859-1') for k in get_attrs.fields}
...
if __name__ == '__main__':
    ...
    import alignface
    face_d, face_p = alignface.load_face_detector(predictor_path='../facemodel/shape_predictor_68_face_landmarks.dat')
