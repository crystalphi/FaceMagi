...
def dfi_reconstruct(F2, net, blob_names, caffe_in, max_iter=2000, verbose=False, tv_lambda=0.001, tv_beta=2, bounds=(-128, 162)):
    ...
    #solver_param={'maxiter': max_iter, 'iprint': 1}
    loss0, _ = deepart.objective_func(
        x0, net, all_target_blob_names, targets, target_data_list, tv_lambda, tv_beta)
    opt_res = scipy.optimize.minimize(deepart.objective_func, x0, args=(net, all_target_blob_names, targets, target_data_list, tv_lambda, tv_beta), bounds=zip(
        numpy.full_like(x0, bounds[0]), numpy.full_like(x0, bounds[1])), method=solver_type, jac=True, options=solver_param)
    loss1, _ = deepart.objective_func(
        opt_res.x, net, all_target_blob_names, targets, target_data_list, tv_lambda, tv_beta)

    data = opt_res.x.reshape(*caffe_in.shape)
    if verbose:
        # print(opt_res)
        print('reconstruct finished in {} minutes. loss: {} -> {}'.format(
            (time.time()-t0)/60.0, loss0, loss1))
    return data
...
def caffe_mean_F(X, net, input_blob, blob_names, mean, scale):
    ...
    n = 0
    for x in X:
        caffe_in = ((x[numpy.newaxis, :, :, ::-1]*255-mean)
...
def caffe_hypercolumn(F, blob_names):
    k0 = blob_names[0]
    shape = (F[k0].shape[0], sum(F[k].shape[1]
                                 for k in blob_names), F[k0].shape[2], F[k0].shape[3])
    result = [F[k0]]
    for k in blob_names[1:]:
        zoom = (1.0, 1.0, float(F[k0].shape[2]) /
                F[k].shape[2], float(F[k0].shape[3])/F[k].shape[3])
        result.append(scipy.ndimage.interpolation.zoom(
            F[k], zoom, order=1, mode='nearest'))
...
class vgg19g(object):
    ...
    def F_inverse(self, F, initial_image, **options):
        ...
        caffe_in = numpy.expand_dims(
            (initial_image[:, :, ::-1]*255-self.mean).transpose(2, 0, 1), 0)
        F2 = caffe_unflatten(F, self.net, caffe_in, self.blob_names)
        data = dfi_reconstruct(F2, self.net, self.blob_names,
                               caffe_in, tv_lambda=tv_lambda, max_iter=max_iter)
...
class celeba_attributes(object):
    ...
    def select(self, constraints, attributes, **options):
        ...
        S = numpy.asarray(
            [i for i in range(len(self.filelist)) if admissible(i)])
        knn = numpy.argsort(scipy.spatial.distance.cdist(numpy.expand_dims(
            attributes[self.distance_idx], 0), self._binary_score[S][:, self.distance_idx], 'minkowski', 1)).astype(numpy.int32)
...
class facemodel_attributes(object):
    ...
    def __init__(self, **options):
        data = numpy.load(os.path.join(
            script_dir, 'datasets/facemodel/attributes.npz'))
        ...
        self._confident[self._scores > numpy.percentile(
            self._scores[self._scores >= 0], 30, axis=0)] = True
        self._confident[self._scores < numpy.percentile(
            self._scores[self._scores < 0], 70, axis=0)] = True
        self.map_name = {k: i for i, k in enumerate(self.filelist)}
        self.rev_map_name = {i: k for i, k in enumerate(self.filelist)}
        ...
        self.facemodel_worker = FacemodelWorker()
    ...
    def raise_remote_errors(self, args, kwargs):
        if len(args) >= 1 and args[0] == 'fail':
            if 'traceback' in kwargs:
                ...
                raise RuntimeError(
                    'remote command failed (no traceback available)')
    ...
    def score(self, X, **options):
        if 'landmarks' in options:
            scores = self.facemodel_worker.forward_landmarks(X=pickle.dumps(X), Xlm=pickle.dumps(landmarks))
        else:
            scores = self.facemodel_worker.forward_images(X=pickle.dumps(X))
        return pickle.loads(scores) #?? or return scores??
...
from . import alignface
#os.chdir(os.path.join(script_dir, 'models/facemodel'))
from .models.facemodel import facemodel

class FacemodelWorker(object):

    def __init__(self, **options):
        self.fm = facemodel.FaceModel()
        self.face_d, self.face_p = alignface.load_face_detector(
            predictor_path='models/facemodel/shape_predictor_68_face_landmarks.dat')
    
    def forward_landmarks(self, X, Xlm):
        ...
        for x, lm in zip(X, Xlm):
            ...
            M, loss = alignface.fit_face_landmarks(
                lm, self.fm.celeba_template, image_dims=[160, 160], twoscale=False)
            Y = alignface.warp_to_template(x, M, image_dims=[160, 160])
            Y = numpy.expand_dims(Y, 0)
            Y = self.fm.preprocess(Y)
            Y = self.fm.forward(Y)
            scores = self.fm.predict_attributes(Y)
    ...
    def forward_images(self, X):
        ...
        X = [(x if os.path.isabs(x) else os.path.join('../..', x))
              for x in X]
        result = []
        for ipath in X:
            lm, x = alignface.detect_landmarks(ipath, self.face_d, self.face_p)
            M, loss = alignface.fit_face_landmarks(
                lm, self.fm.celeba_template, image_dims=[160, 160], twoscale=False)
            Y = alignface.warp_to_template(x, M, image_dims=[160, 160])
            Y = numpy.expand_dims(Y, 0)
...
if __name__ == '__main__':
    ...
    '''
    classifier=facemodel_attributes()
    print(len(classifier.fields()))
    print(classifier.fields()[0])
    print(classifier.lookup_scores(['images/facemodel/celeba/000009.jpg'])[0,:5])
    print(numpy.allclose(classifier.score(['tests/000009.jpg'])[0,:5],[-2.39323288,  0.5700731 ,  0.917334  , -0.66668013, -2.66346439]))
