...
def load_face_detector(predictor_path='models/facemodel/shape_predictor_68_face_landmarks.dat'):
...
def detect_landmarks(ipath,detector,predictor,upsample=0,image=None):
  if image is None:
    ...
    from app.engine_v1.utils import pil_to_skimage
    original255=pil_to_skimage(image).astype(numpy.ubyte)
    original=original255/255.0
    #print('--> image:', image)
    #print('--> original:', original)
    #print('--> original255:', original255)
  dets=detector(original255,upsample)
  #if len(dets)!=1: raise FitError('{}: detected zero or more than one face.'.format(ipath))
  if len(dets)<1: raise FitError('{}: no face detected.'.format(ipath))
