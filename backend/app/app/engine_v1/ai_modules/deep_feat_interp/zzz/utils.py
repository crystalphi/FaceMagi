...
def warped_image_feed(S, MP, image_dims):
    ...
    for i, x in enumerate(S):
        x = os.path.join(_dir, '..', x.decode("utf-8"))
        I = imageutils.read(x) #TODO: 从读硬盘改为预加载到内存
...
def color_match(A, B):
    ...
    m2 = numpy.expand_dims(numpy.expand_dims(
        numpy.expand_dims(m2, -1), -1), -1)
    s2 = (B-m2).reshape(B.shape[0], B.shape[1], -1).std(axis=2)
    s2 = numpy.expand_dims(numpy.expand_dims(
        numpy.expand_dims(s2, -1), -1), -1)
