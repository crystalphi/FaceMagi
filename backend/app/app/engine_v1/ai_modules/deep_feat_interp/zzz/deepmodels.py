...
class facemodel_attributes(object):
    ...
    def __init__(self, **options):
        data = numpy.load(os.path.join(
            app_config.MODEL_DIR, 'deep_feat_interp/attributes.npz'))
    ...
    def score(self, X, **options):
        if 'landmarks' in options:
            scores = self.facemodel_worker.forward_landmarks(X, Xlm=landmarks)
        else:
            scores = self.facemodel_worker.forward_images(X)
        return scores
...
class FacemodelWorker(object):
    ...
    def forward_images(self, X):
        ...
        print('--> type(X)', type(X))
        for img in X:
            lm, x = alignface.detect_landmarks(None, self.face_d, self.face_p, image=img)
            M, loss = alignface.fit_face_landmarks(
                lm, self.fm.celeba_template, image_dims=[160, 160], twoscale=False)
            Y = alignface.warp_to_template(x, M, image_dims=[160, 160])
            Y = numpy.expand_dims(Y, 0)
