            ...
            if command == 'ping':
                ...
                for x, lm in zip(X, Xlm):
                    ...
                    M, loss = alignface.fit_face_landmarks(
                        lm, fm.celeba_template, image_dims=[160, 160], twoscale=False)
                ...
                channel.send(
                    (('okay', responseid), {'scores': pickle.dumps(result)}))
...
if __name__ == '__main__':
    ...
    Xlm, X = alignface.detect_landmarks(
        'images/jpg-816x1000/vvdd-11.jpg', face_d, face_p)
    X = numpy.expand_dims(X, 0)
    Xlm = numpy.expand_dims(Xlm, 0)
    channel.send((('forward_landmarks', 1), {
                 'X': pickle.dumps(X), 'Xlm': pickle.dumps(Xlm)}))
    print('gw', gw.remote_status())
    args, kwargs = channel.receive()
    print('recv', args[0], args[1], [pickle.loads(x) for x in args[2:]], [
          '{} = {}'.format(k, pickle.loads(v)) for (k, v) in kwargs.iteritems()])
    channel.send((('forward_images', 2), {
                 'X': pickle.dumps(['images/jpg-816x1000/vvdd-11.jpg'])}))
    print('gw', gw.remote_status())
    args, kwargs = channel.receive()
    print('recv', args[0], args[1], [pickle.loads(x) for x in args[2:]], [
          '{} = {}'.format(k, pickle.loads(v)) for (k, v) in kwargs.iteritems()])
